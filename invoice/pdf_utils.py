...
def process_invoice(data):
    """Original function that saves PDF to disk"""
...
# 🔹 NEW FUNCTION FOR VERCEL (returns BytesIO)
def process_invoice_bytes(data):
    """Generate PDF in memory, return BytesIO for Vercel deployment"""
    if not os.path.exists(ORIGINAL_PDF):
        raise FileNotFoundError("Original PDF template not found in 'uploads/' folder.")
